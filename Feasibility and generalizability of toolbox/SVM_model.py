...
def SVM(data_root_path,model_save,test_result_txt,train_result_txt,row_patch_size,col_patch_size,channel,gamma,C):
    ...
    joblib.dump(predictor,model_save+'\SVM_g_'+str(gamma)+'_C_'+str(C)+'.model')
...
gamma = arcpy.GetParameterAsText(2)
...
for i in gamma:
    for j in C:
        model_save = model_save_path+'/g_'+str(i)+'_C_'+str(j)
        if not os.path.exists(model_save):
            os.makedirs(model_save)
        train_result_txt = model_save+'/SVM_train_result_txt.txt'
        test_result_txt = model_save+'/SVM_test_result_txt.txt'
        roc_auc, testacc, trainacc = SVM(data_root_path, model_save,test_result_txt, train_result_txt, row_patch_size,col_patch_size, channel, i, j)
        with open(result_txt, "a") as f:
            line = "%f\t%f\t%f\t%f\t%f\t%f\n" % (i, j, roc_auc, testacc, trainacc,trainacc-testacc)
            f.write(line)
        arcpy.AddMessage("********************gamma=" + str(i) + "," + "C=" + str(j) + "SVM train over!********************")
